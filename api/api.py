...
def run_financial_simulation(initial_income_param, initial_expenditure_param, initial_capital_param, current_age_param, future_age_param):
    ...
    for year_idx in range(1, years_to_simulate + 1):
        ...
        if current_sim_age > 35:
            career_advancement_prob *= max(0.1, 1 - (current_sim_age - 35) * 0.02)
        ...
        if job_loss_active_months <= 0 and random.random() < career_advancement_prob:
            ...
            current_income_annual = min(current_income_annual, 150)
        ...
        if job_loss_active_months <= 0 and job_loss_recovery_years_remaining <=0 :
            if current_income_annual >= 150:
                pass 
            elif current_income_annual >= 100:
                current_income_annual *= random.uniform(1.005, 1.015)
